...
public class Calculation {
    ...
    public double ScalarVec(Vector3D v1, Vector3D v2){ 
    ...
    // Area of Triangle
    public double Area_Triangle(ArrayList<Point> p){
        Point A = p.get(0);
        Point B = p.get(1);
        Point C = p.get(2);
        ...
        return (double) Math.sqrt(peri*(peri - AB)*(peri - BC)*(peri - CA));//herong 
    }
    
    // Area of Quadrilateral (tứ giác)
    public double Area_Quadrilateral(ArrayList<Point> p){
        Point A = p.get(0);
        Point B = p.get(1);
        Point C = p.get(2);
        Point D = p.get(3);
    ...
    // Volume of pyramid
    public double Volume_Pyramid(ArrayList<Point> p,Point S){
    ...
    public double PointToPlane(Point A, Plane P){
        double dis = (double) Math.sqrt(P.getA()*P.getA()+P.getB()*P.getB()+P.getC()*P.getC());
